...
public class BombsStartegy implements ObjectsFallingStrategy {
    ...
    public ArrayList<GameObject> generateObjectsFalling(int n) {
        ...
        {
            if(Math.random()<=0.2)
               moving.add(factory.createObject("Bomb",(int)(Math.random()*1400/*getWidth()*/), (int)(Math.random()*750/10 /*getHeight()/2*/),"FinalProjectt\\bombresized.png"));
            else if (Math.random()<=0.1) {
                moving.add(factory.createObject("Heart",(int)(Math.random()*1400/*getWidth()*/), (int)(Math.random()*750/2) , "FinalProjectt\\clown-removebg-preview_3_53.png"));
            }
            else{
